...
def plot_hydropathy_list(hydropathy_sequence_list, acc_uniprot, length_sliding_window):
    ...
    sequence_position_hydropathy_df = pd.DataFrame(sequence_position_hydropathy_tuple,
                                                   columns=['Sequence Position', 'Hydropathy'])

    hydropathy_plot = px.bar(sequence_position_hydropathy_df, x='Sequence Position', y='Hydropathy',
                             title="Hydropathy List for Uniprot Acc: " + acc_uniprot + " with a sliding window of " + str(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fasta_file", help="FASTA file with the sequences to be analyzed.", type=str, default="NaN",
                        required=False)
